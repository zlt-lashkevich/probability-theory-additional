...
#box(
  ...
  *Перекрытие $X → Y$:* Суффикс паттерна $X$ совпадает с префиксом паттерна $Y$.
  
  *Смысл:* После появления $X$, паттерн $Y$ может появиться "быстрее", используя общую часть.
]
...
- $P(B) dot T H$: Самоперекрытие $H T H → H T H$: суффикс $H$ = префикс $H$, добавляем $T H$
- $P(A) dot T H$: Перекрытие $T H H → H T H$: суффикс $H$ = префикс $H$, добавляем $T H$

== *Для паттерна $T H H$:*

$
  "Traj" dot T H H = P(B) + P(A) dot H 
$

- $P(B)$: Чистая победа $B$: паттерн $T H H$ появился первым
- $P(A) dot H$: Перекрытие $H T H → T H H$: суффикс $T H$ = префикс $T H$, добавляем $H$
...
#box(
  ...
  inset: 10pt,
  radius: 4pt,
)[Паттерн $T H H$ *не имеет самоперекрытий*! Это критическое отличие от $H T H$, который перекрывается сам с собой.
]
...
#box(
  ...
  *Уравнение 1 (для $H T H$):*
  $ Lambda = P(A) + P(A) dot 1/4 + P(B) dot 1/4 $
  $ Lambda = P(A) (1 + 1/4) + P(B) dot 1/4 = 5/4 P(A) + 1/4 P(B) $
  
  *Уравнение 2 (для $T H H$):*
  $ Lambda = P(B) + P(A) dot 1/2 $
  
  *Уравнение 3 (нормировка):*
  $ P(A) + P(B) = 1 $
]

Приравниваем уравнения 1 и 2

$ 5/4 P(A) + 1/4 P(B) = P(B) + 1/2 P(A) $

Переносим $P(A)$ влево, $P(B)$ вправо:

$ 5/4 P(A) - 1/2 P(A) = P(B) - 1/4 P(B) quad quad (5/4 - 2/4) P(A) = (4/4 - 1/4) P(B) $

$ 3/4 P(A) = 3/4 P(B) quad quad  P(A) = P(B) $

Применяем нормировку

$ P(A) + P(B) = 1, quad P(A) = P(B) quad quad  2 P(A) = 1 $
...
#box(
  ...
  #align(center)[
    *Ответ:*
    ...
    Игра *честная*: оба игрока имеют равные шансы!
  ]
]
